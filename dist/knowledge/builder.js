...
exports.KnowledgeBuilder = void 0;
exports.buildKnowledgeBase = buildKnowledgeBase;
const cheerioParser_1 = require("../fetchers/cheerioParser");
const moodleHtmlFetcher_1 = require("../fetchers/moodleHtmlFetcher");
const pageNavigator_1 = require("../fetchers/pageNavigator");
...
class KnowledgeBuilder {
    constructor() {
        this.htmlFetcher = new moodleHtmlFetcher_1.MoodleHtmlFetcher();
        this.cheerioParser = new cheerioParser_1.CheerioParser();
        this.pageNavigator = new pageNavigator_1.PageNavigator();
        this.knowledgeInserter = new insert_1.KnowledgeInserter();
    }
    /**
     * 从 Moodle 课程构建知识库
     ...
     */
    async buildFromCourses(courseIds, clearExisting = false) {
        logger_1.logger.info(`Starting knowledge base build for ${courseIds.length} courses`);
        for (const courseId of courseIds) {
            try {
                ...
                logger_1.logger.error(`Error processing course ${courseId}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        // 关闭 PageNavigator 浏览器
        await this.pageNavigator.close();
        logger_1.logger.info('Knowledge base build completed');
    }
    /**
     * 处理单个课程
     * @param courseId - 课程 ID
     */
    async processCourse(courseId) {
        try {
            // 获取课程页面 HTML
            ...
            for (const section of course.sections) {
                for (const item of section.items) {
                    try {
                        ...
                        logger_1.logger.error(`Error processing item ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
            }
            logger_1.logger.info(`Processed course: ${course.name} (${courseId})`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process course ${courseId}: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    /**
     ...
     * @param course - 课程对象
     */
    async saveBasicCourseInfo(course) {
        // 构建课程概述内容
        ...
        await this.knowledgeInserter.insertOrUpdate({
            ...
            type: types_1.KnowledgeType.COURSE_INFO,
            content,
            course_id: course.id,
     ...
     */
    async processContentItem(item, courseName) {
        // 根据内容类型进行处理
        switch (item.type) {
            case cheerioParser_1.MoodleContentType.ASSIGNMENT:
                await this.processAssignment(item, courseName);
                break;
            case cheerioParser_1.MoodleContentType.FORUM:
                await this.processForum(item, courseName);
                break;
            case cheerioParser_1.MoodleContentType.RESOURCE:
                await this.processResource(item, courseName);
                break;
            case cheerioParser_1.MoodleContentType.QUIZ:
     ...
     * @param courseName - 课程名称
     */
    async processAssignment(item, courseName) {
        try {
            ...
            await this.knowledgeInserter.insertOrUpdate({
                title: item.name,
                type: types_1.KnowledgeType.ASSIGNMENT,
                content,
            ...
            logger_1.logger.info(`Processed assignment: ${item.name}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process assignment ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 处理论坛内容
     ...
     * @param courseName - 课程名称
     */
    async processForum(item, courseName) {
        try {
            // 获取论坛 HTML
            ...
            await this.knowledgeInserter.insertOrUpdate({
                title: item.name,
                type: types_1.KnowledgeType.FORUM_POST,
                content,
                html_content: htmlContent,
                course_id: item.courseId,
                course_name: courseName,
            ...
            logger_1.logger.info(`Processed forum: ${item.name}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process forum ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 处理资源内容
     ...
     * @param courseName - 课程名称
     */
    async processResource(item, courseName) {
        try {
            // 获取资源 HTML
            ...
            await this.knowledgeInserter.insertOrUpdate({
                title: item.name,
                type: types_1.KnowledgeType.RESOURCE,
                content,
                html_content: htmlContent,
                course_id: item.courseId,
                course_name: courseName,
            ...
            logger_1.logger.info(`Processed resource: ${item.name}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process resource ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 处理测验内容
     ...
     * @param courseName - 课程名称
     */
    async processQuiz(item, courseName) {
        try {
            ...
            await this.knowledgeInserter.insertOrUpdate({
                title: item.name,
                type: types_1.KnowledgeType.QUIZ,
                content,
                html_content: htmlContent,
                course_id: item.courseId,
                course_name: courseName,
            ...
            logger_1.logger.info(`Processed quiz: ${item.name}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process quiz ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 处理通用内容项
     ...
     * @param courseName - 课程名称
     */
    async processGenericItem(item, courseName) {
        try {
            // 获取通用内容 HTML
            ...
            await this.knowledgeInserter.insertOrUpdate({
                title: item.name,
                type: types_1.KnowledgeType.OTHER,
                content,
                html_content: htmlContent,
                course_id: item.courseId,
                course_name: courseName,
            ...
            logger_1.logger.info(`Processed generic item: ${item.name}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to process generic item ${item.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
...
async function buildKnowledgeBase(options) {
    ...
    logger_1.logger.info('Knowledge base build completed via buildKnowledgeBase function');
}
