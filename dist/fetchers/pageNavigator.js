...
class PageNavigator {
     ...
     * 获取需要额外交互才能访问的作业详情页面
     */
    async getAssignmentDetails(assignmentUrl) {
        if (!this.page) {
            await this.init();
        }
        if (!this.page) {
        ...
        logger_1.logger.info(`Getting assignment details from: ${assignmentUrl}`);
        ...
        if (hasCriteriaButton) {
            logger_1.logger.info('Clicking on "View grading criteria" button');
            ...
            await this.page.waitForSelector('.gradingmethodpreview', { timeout: 5000 }).catch(() => {
                logger_1.logger.warn('Could not find grading criteria after clicking button');
            });
        }
        return await this.page.content();
    }
     ...
     * 获取需要额外交互才能访问的论坛帖子详情
     */
    async getForumPostDetails(forumDiscussionUrl) {
        if (!this.page) {
            await this.init();
        }
        if (!this.page) {
        ...
        logger_1.logger.info(`Getting forum post details from: ${forumDiscussionUrl}`);
    ...
    async getQuizDetails(quizUrl) {
        ...
        logger_1.logger.info(`Getting quiz details from: ${quizUrl}`);
        ...
        if (hasPreviewButton) {
            logger_1.logger.info('Found quiz preview button, but not clicking to avoid starting quiz');
        }
        return await this.page.content();
    }
}
