...
exports.CheerioParser = exports.MoodleContentType = void 0;
const cheerio = __importStar(require("cheerio"));
const logger_1 = require("../utils/logger");
/**
 * Moodle 课程中的内容类型
 */
var MoodleContentType;
(function (MoodleContentType) {
    MoodleContentType["ASSIGNMENT"] = "assignment";
    MoodleContentType["RESOURCE"] = "resource";
    MoodleContentType["URL"] = "url";
    MoodleContentType["FORUM"] = "forum";
    MoodleContentType["QUIZ"] = "quiz";
    MoodleContentType["FOLDER"] = "folder";
    MoodleContentType["ANNOUNCEMENT"] = "announcement";
    MoodleContentType["OTHER"] = "other";
})(MoodleContentType || (exports.MoodleContentType = MoodleContentType = {}));
/**
...
class CheerioParser {
     ...
     */
    parseCoursePageHtml(html, courseId) {
        const $ = cheerio.load(html);
        const courseName = $('.page-header-headings h1').text().trim();
        logger_1.logger.info(`Parsing course page: ${courseName}`);
        const sections = [];
        // 解析课程章节
        $('.section.main').each((i, sectionEl) => {
            ...
            $(sectionEl).find('li.activity').each((j, activityEl) => {
                try {
                    ...
                    logger_1.logger.error(`Error parsing activity: ${error instanceof Error ? error.message : String(error)}`);
                }
            });
            sections.push({
                id: sectionId,
        ...
        logger_1.logger.info(`Parsed ${sections.length} sections with ${sections.reduce((acc, section) => acc + section.items.length, 0)} total items`);
        return {
            id: courseId,
            name: courseName,
            sections
